...
for (const candidate of plan.routes) {
  ...
  const token = web?.tokens?.[candidate.tokenIndex];
  ...
  if (normalized !== candidate.expectedNormalized) {
    routeResult.errors.push({
      reference: candidate.reference,
      tokenIndex: candidate.tokenIndex,
      reason: "target-token-mismatch",
      expected: candidate.expectedNormalized,
      actual: normalized
    });
    continue;
  }
  ...
  const webOccurrence = occurrenceOrdinal(
    web.tokens,
    normalized,
  ...
  const kjvMatches = (kjv.tokens ?? [])
    .map((parallelToken, index) => ({
      index,
      token: parallelToken
    }))
    .filter(item => normalizedToken(item.token) === normalized);

  if (
    webOccurrence.ordinal < 0 ||
    webOccurrence.matchingIndexes.length !== kjvMatches.length
  ) {
    routeResult.errors.push({
      reference: candidate.reference,
      tokenIndex: candidate.tokenIndex,
  ...
  const parallelRoutes = routeIds(
    kjvMatches[webOccurrence.ordinal].token
  );
  ...
  if (
    ...
    routeResult.errors.push({
      reference: candidate.reference,
      tokenIndex: candidate.tokenIndex,
      reason: "parallel-route-mismatch",
      parallelRoutes,
...
if (routeResult.errors.length > 0) {
  throw new Error(
    `Route restoration failed for ${routeResult.errors.length} targets.`
  );
}
