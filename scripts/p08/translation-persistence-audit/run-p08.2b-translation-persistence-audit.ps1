...
$phaseRoot = Join-Path $repo ".private\reports\P08.2B-AUDIT"
$reportRoot = Join-Path $phaseRoot "$stamp-translation-persistence-audit"
$successZip = Join-Path $phaseRoot "EMETSEES-P08.2B-TRANSLATION-PERSISTENCE-AUDIT-$stamp.zip"
$failureZip = Join-Path $phaseRoot "EMETSEES-P08.2B-TRANSLATION-PERSISTENCE-AUDIT-FAILURE-$stamp.zip"
$nodeScript = Join-Path $PSScriptRoot "audit-p08.2b-translation-persistence.cjs"
...
try {
    if (-not (Test-Path -LiteralPath $nodeScript -PathType Leaf)) {
        throw "The P08.2B audit script is missing: $nodeScript"
    }
    ...
    if ($nodeCheckExit -ne 0) {
        ...
        throw "The P08.2B Node audit script failed syntax validation."
    }
    ...
    if ($auditExit -ne 0) {
        throw "The P08.2B translation persistence audit failed with exit code $auditExit."
    }

    New-ReportZip -SourceDirectory $reportRoot -DestinationZip $successZip

    Write-Host ""
    Write-Host "P08.2B translation persistence audit completed." -ForegroundColor Green
    ...
    $failureText = @(
        "P08.2B translation persistence audit failure",
    ...
    try {
        ...
        Write-Host ""
        Write-Host "P08.2B audit failed. Failure ZIP:" -ForegroundColor Yellow
