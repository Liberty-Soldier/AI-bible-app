...
$reportName = "$timestamp-brenton-integrity-build-blocker-diagnostic"
$reportDir = Join-Path $RepositoryRoot ".private\reports\P05.12\$reportName"
$scriptPath = Join-Path $RepositoryRoot "scripts\p0512\diagnose-brenton-integrity-build-blocker.cjs"
$zipName = "EMETSEES-P0512AP-B1-BRENTON-INTEGRITY-BUILD-BLOCKER-DIAGNOSTIC-$timestamp.zip"
...
Write-Host ""
Write-Host "==> Verifying rollback and diagnosing the existing Brenton integrity gate"
& node --max-old-space-size=8192 $scriptPath --repo $RepositoryRoot --output $reportDir 1> $stdout 2> $stderr
$nodeStatus = $LASTEXITCODE

# Build a Windows PowerShell 5.1-compatible checksum manifest.
...
[System.IO.File]::WriteAllLines((Join-Path $reportDir "checksums.sha256"), $checksumLines, (New-Object System.Text.UTF8Encoding($false)))
