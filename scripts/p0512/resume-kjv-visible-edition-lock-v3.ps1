...
$relativeOutput = $outputRoot.Substring($RepoRoot.Length)
...
$resumeSummary |
    ...
    Set-Content `
        -LiteralPath (
            Join-Path $outputRoot "p0512z-resume-v3-summary.json"
        ) `
        -Encoding UTF8

$readmePath = Join-Path $outputRoot "README-RESUME-V3.md"
$readme = @"
# P05.12Z Resume V3
...
Resume V3 reused the successful reconciliation and decision output, rechecked
...
$reportZip = Join-Path `
    $reportParent `
    "EMETSEES-P0512Z-KJV-VISIBLE-EDITION-LOCK-RESUME-V3-$stamp.zip"
...
Write-Host ""
Write-Host "P05.12Z Resume V3 completed." -ForegroundColor Green
