...
function relativeFileSet(root) {
  return listFilesRecursive(root).map((file) => relativeFromRoot(root, file)).sort();
}
...
function main() {
  ...
  const gates = {
    ...
    akStillMatchesValidFailClosedState: ak.passed,
  ...
  const summary = {
    ...
    counts: sa.counts,
    selectedFlows: sa.selectedFlows,
    gates,
    authorization: {
  ...
  writeJson(path.join(reportDir, "p0512al-summary.json"), summary);
