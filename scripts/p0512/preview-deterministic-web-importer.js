...
function fail(message) {
  throw new Error(`[P05.12E WEB importer preview] ${message}`);
}
...
function main() {
  ...
  const profile = lockedProfiles.translations?.web;
  if (!profile || profile.validationPassed !== true) {
    fail("The locked WEB source profile is missing or invalid.");
  }

  const certifiedPath = findLatestCertifiedCensus();
  const certified = readJson(certifiedPath);
  const certifiedWeb = certified.translations?.web;
  if (certified.milestone !== "P05.12D" || !certifiedWeb) {
    fail("Latest certified census does not contain a valid WEB result.");
  }

  const sourceDirectory = toAbsoluteRepoPath(profile.rawSourcePath);
  const currentPath = path.join(ROOT, "app", "data", "scripture", "generatedWEB.json");

  if (!fs.existsSync(sourceDirectory)) fail(`Locked WEB source is missing: ${sourceDirectory}`);
  if (!fs.existsSync(currentPath)) fail(`Current WEB reader file is missing: ${currentPath}`);

  const currentHashBefore = sha256File(currentPath);
  if (currentHashBefore !== certifiedWeb.current.sha256) {
    fail(
      `generatedWEB.json changed after P05.12D. Expected ${certifiedWeb.current.sha256}, found ${currentHashBefore}`,
    );
  }

  console.log("[P05.12E] Re-verifying immutable WEB source tree...");
  const actualTreeHash = computeTreeSha256(sourceDirectory);
  if (actualTreeHash !== profile.rawTreeSha256) {
    fail(
      `Locked WEB source tree hash changed. Expected ${profile.rawTreeSha256}, found ${actualTreeHash}`,
    );
  }
  ...
  console.log("[P05.12E] Parsing locked WEB USFM...");
  const source = parseUsfmDirectory(sourceDirectory, "web", includedIds);

  console.log("[P05.12E] Loading current WEB reader schema...");
  const currentDocument = readJson(currentPath);
  const current = loadCurrentTranslation(currentPath, "web", allowedBooks);

  if (current.invalid.length || current.duplicates.length || current.excludedProfileRecords.length) {
    fail(
      `Current WEB reader schema failed preflight: ${JSON.stringify({
  ...
  const expected = {
    records: certifiedWeb.current.records,
    sourceVerses: certifiedWeb.source.simpleNumericVerses,
    headings: certifiedWeb.source.headings,
    footnotes: certifiedWeb.source.footnotes,
    crossReferences: certifiedWeb.source.crossReferences,
    strongTags: certifiedWeb.source.strongTags,
    emptySourceVerseLabels: certifiedWeb.source.emptySimpleVerseLabels,
    substantiveDifferences: certifiedWeb.comparison.substantiveDifferences,
    netSourceWordDeficit: certifiedWeb.comparison.netSourceWordDeficit,
  };

  const actualSourceCounts = {
    records: current.records.length,
    sourceVerses: source.verses.filter(
      (verse) => !verse.isBridge && !verse.isSubverse && verse.wordCount > 0,
    ).length,
    headings: source.headings.length,
    footnotes: source.verses.reduce((sum, verse) => sum + verse.footnotes.length, 0),
    crossReferences: source.verses.reduce(
      (sum, verse) => sum + verse.crossReferences.length,
      0,
    ),
    strongTags: source.verses.reduce(
      (sum, verse) => sum + verse.wordMetadata.length,
      0,
    ),
    emptySourceVerseLabels: source.emptyVerses.length,
  };

  for (const key of ["records", "sourceVerses", "headings", "footnotes", "crossReferences", "strongTags", "emptySourceVerseLabels"]) {
    if (actualSourceCounts[key] !== expected[key]) {
      fail(
        `WEB certified count drift for ${key}: expected ${expected[key]}, found ${actualSourceCounts[key]}`,
      );
    }
  }

  console.log("[P05.12E] Building deterministic staged artifacts twice...");
  const buildOne = buildWebArtifacts({
    profile,
    profilesPath,
    certifiedPath,
    certifiedSummary: certified,
    currentPath,
    currentDocument,
    current,
    source,
  ...
  if (buildOne.deterministicFingerprint !== buildTwo.deterministicFingerprint) {
    fail(
      `WEB importer is not deterministic: ${buildOne.deterministicFingerprint} != ${buildTwo.deterministicFingerprint}`,
    );
  }
  ...
  const stagedMetadataCounts = {
    headings: buildOne.headingRows.length,
    structureEvents: buildOne.structureRows.length,
    footnotes: buildOne.footnoteRows.length,
    crossReferences: buildOne.crossReferenceRows.length,
    strongTags: stagedStrongTagCount,
    emptySourceVerseLabels: buildOne.sourceOnlyVerseRows.length,
  };

  const expectedMetadataCounts = {
    headings: actualSourceCounts.headings,
    structureEvents: source.structures.length,
    footnotes: actualSourceCounts.footnotes,
    crossReferences: actualSourceCounts.crossReferences,
    strongTags: actualSourceCounts.strongTags,
    emptySourceVerseLabels: actualSourceCounts.emptySourceVerseLabels,
  };

  for (const key of Object.keys(expectedMetadataCounts)) {
    if (stagedMetadataCounts[key] !== expectedMetadataCounts[key]) {
      fail(
        `WEB staged metadata loss for ${key}: expected ${expectedMetadataCounts[key]}, found ${stagedMetadataCounts[key]}`,
      );
    }
  }

  if (buildOne.changedRows.length !== expected.substantiveDifferences) {
    fail(
      `WEB changed-verse count drift: expected ${expected.substantiveDifferences}, found ${buildOne.changedRows.length}`,
    );
  }
  ...
  console.log(`[P05.12E] Writing staged artifacts to ${relative(ROOT, stagingRoot)}...`);
  const stagedFiles = writeStagedArtifacts(stagingRoot, buildOne);

  console.log("[P05.12E] Running candidate-to-source zero-difference audit...");
  const candidate = loadCurrentTranslation(
    stagedFiles.readerCandidate.path,
    "web-candidate",
    allowedBooks,
  );
  const candidateAudit = compareTranslation("web-candidate", candidate, source);
  ...
  if (
    ...
    candidateAudit.summary.comparison.exact !== expected.sourceVerses
  ) {
    fail(
      `Candidate/source audit failed: ${JSON.stringify(candidateAudit.summary.comparison, null, 2)}`,
    );
  }
  ...
  if (currentHashAfter !== currentHashBefore) {
    fail(
      `Production generatedWEB.json changed during preview: ${currentHashBefore} -> ${currentHashAfter}`,
    );
  }

  const currentAudit = compareTranslation("web-current", current, source);
  if (
    currentAudit.summary.comparison.substantiveDifferences !== expected.substantiveDifferences ||
    currentAudit.summary.comparison.netSourceWordDeficit !== expected.netSourceWordDeficit
  ) {
    fail(
      `Certified current/source census could not be reproduced: ${JSON.stringify(currentAudit.summary.comparison, null, 2)}`,
    );
  }
  ...
  const stagingManifest = {
    ...
    inputs: {
      authoritativeProfilesPath: relative(ROOT, profilesPath),
      authoritativeProfilesSha256: sha256File(profilesPath),
      certifiedCensusPath: relative(ROOT, certifiedPath),
      certifiedCensusSha256: sha256File(certifiedPath),
      currentReaderPath: relative(ROOT, currentPath),
      currentReaderSha256: currentHashBefore,
    },
    output: {
      stagingRoot: relative(ROOT, stagingRoot),
    ...
    counts: {
      ...
      footnotes: buildOne.footnoteRows.length,
      crossReferences: buildOne.crossReferenceRows.length,
      versesWithStrongsMetadata: buildOne.strongRows.length,
      strongTags: stagedStrongTagCount,
      sourceOnlyEmptyVerseLabels: buildOne.sourceOnlyVerseRows.length,
      versificationMappings: buildOne.versificationRows.length,
      currentSubstantiveDifferences: currentAudit.summary.comparison.substantiveDifferences,
      currentConfirmedMissingWords:
        currentAudit.summary.comparison.confirmedMissingWordOccurrences,
      currentNetWordDeficit:
        currentAudit.summary.comparison.netSourceWordDeficit,
      candidateSubstantiveDifferences:
        candidateAudit.summary.comparison.substantiveDifferences,
    },
    gates: {
      sourceTreeImmutable: true,
      sourceProfileValid: source.profileValidation.passed,
      currentReaderHashMatchesCertifiedCensus: true,
      importerDeterministicAcrossTwoBuilds: true,
      candidateVerseInventoryExact: true,
      candidateVisibleTextExactToSource: true,
      metadataInventoryExactToSource: true,
      sourceOnlyEmptyVerseLabelsPreserved: true,
      productionReaderModified: false,
      displayTokensModified: false,
      alignmentsModified: false,
      safeToApplyProductionReader: false,
      reason:
        "The staged candidate must be reviewed from the P05.12E ZIP before an explicit apply tool is created.",
    },
  };

  writeJson(path.join(stagingRoot, "staging-manifest.json"), stagingManifest);

  writeJson(path.join(args.output, "staging-manifest.json"), stagingManifest);
  writeJson(
    path.join(args.output, "validation-summary.json"),
    {
      milestone: "P05.12E",
      status: "web-importer-preview-passed",
      currentReader: currentAudit.summary,
      candidateReader: candidateAudit.summary,
      deterministicFingerprint: buildOne.deterministicFingerprint,
      currentReaderSha256Before: currentHashBefore,
      currentReaderSha256After: currentHashAfter,
      stagingRoot: relative(ROOT, stagingRoot),
    },
  );

  writeCsv(
    path.join(args.output, "web-current-to-staged-differences.csv"),
    buildOne.changedRows,
    [
      "reference",
      "classification",
      "severity",
  ...
  fs.writeFileSync(
    path.join(args.output, "samples", "web-psalm-4-preview.md"),
    makePsalm4Preview(buildOne),
    "utf8",
  );
  fs.writeFileSync(
    path.join(args.output, "samples", "web-highest-restorations.md"),
    topRestorationsMarkdown(buildOne.changedRows),
    "utf8",
  );
  fs.writeFileSync(
    path.join(args.output, "samples", "web-source-only-empty-verse-labels.md"),
    sourceOnlyVerseLabelsMarkdown(buildOne),
    "utf8",
  );

  const readme = [
    "# EMETSEES P05.12E Deterministic WEB Importer Preview",
    "",
    `Generated: ${new Date().toISOString()}`,
    "",
    "The locked WEB USFM source was parsed into separate staged artifacts for visible Scripture, headings and superscriptions, paragraph and poetry structure, footnotes, cross-references, Strong's metadata, and source-to-reader versification.",
    "",
    "## Validation",
    "",
    `- Source profile books: ${source.scriptureDocuments.length}`,
    `- Reader/source verses: ${buildOne.visibleRows.length}`,
    `- Current substantive differences: ${currentAudit.summary.comparison.substantiveDifferences}`,
    `- Current confirmed missing words: ${currentAudit.summary.comparison.confirmedMissingWordOccurrences}`,
    `- Current net source word deficit: ${currentAudit.summary.comparison.netSourceWordDeficit}`,
    `- Candidate substantive differences: ${candidateAudit.summary.comparison.substantiveDifferences}`,
    `- Candidate exact source matches: ${candidateAudit.summary.comparison.exact}`,
    `- Source footnotes preserved: ${buildOne.footnoteRows.length}/${actualSourceCounts.footnotes}`,
    `- Source cross-references preserved: ${buildOne.crossReferenceRows.length}/${actualSourceCounts.crossReferences}`,
    `- Source Strong's tags preserved: ${stagedStrongTagCount}/${actualSourceCounts.strongTags}`,
    `- Source-only empty verse labels preserved: ${buildOne.sourceOnlyVerseRows.length}/${actualSourceCounts.emptySourceVerseLabels}`,
    `- Deterministic repeated-build fingerprint: ${buildOne.deterministicFingerprint}`,
    "",
    "## Safety",
    "",
    "- Production `generatedWEB.json` was not modified.",
    "- Display tokens were not rebuilt.",
    "- Alignments were not modified.",
    "- The staged candidate is not authorized for production until this report is reviewed.",
    "",
  ].join("\n");
  ...
  console.log("");
  console.log("[P05.12E] Deterministic WEB importer preview complete.");
  console.log(`STAGING_DIR=${stagingRoot}`);
  console.log(`OUTPUT_DIR=${args.output}`);
  console.log(`[P05.12E] Candidate exact source matches: ${candidateAudit.summary.comparison.exact}`);
  console.log(`[P05.12E] Footnotes preserved: ${buildOne.footnoteRows.length}/${actualSourceCounts.footnotes}`);
  console.log(`[P05.12E] Source-only empty verse labels preserved: ${buildOne.sourceOnlyVerseRows.length}/${actualSourceCounts.emptySourceVerseLabels}`);
  console.log("[P05.12E] Candidate substantive differences: 0");
  console.log("[P05.12E] Production generatedWEB.json modified: NO");
  console.log("[P05.12E] Alignments modified: NO");
}
