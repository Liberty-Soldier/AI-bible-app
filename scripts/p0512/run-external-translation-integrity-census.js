...
function fail(message) {
  throw new Error(`[P05.12B census] ${message}`);
}
...
function parseArgs(argv) {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (arg === "--output" && next) {
      ...
      index += 1;
    } else if (arg === "--acquisition-manifest" && next) {
      args.acquisitionManifest = path.resolve(next);
      index += 1;
    } else {
...
function findLatestAcquisitionManifest(explicitPath) {
  if (explicitPath) {
    if (!fs.existsSync(explicitPath)) {
      fail(`Acquisition manifest not found: ${explicitPath}`);
    }
    return explicitPath;
  }
  ...
  const candidates = walk(
    reportRoot,
    (filePath) => path.basename(filePath) === "source-acquisition-manifest.json",
  );

  if (!candidates.length) {
    fail(
      `No source-acquisition-manifest.json found under ${relative(ROOT, reportRoot)}`,
    );
  }
...
function parseUsfmDirectory(directory, translationId) {
  ...
  const scriptureDocuments = documents.filter(
    (document) => document.bookId && document.verses.length,
  );
  ...
  return {
    ...
    documents,
    scriptureDocuments,
    verses: scriptureDocuments.flatMap((document) => document.verses),
    headings: scriptureDocuments.flatMap((document) => document.headings),
    structures: scriptureDocuments.flatMap((document) => document.structures),
    verseLabels: scriptureDocuments.flatMap((document) => document.verseLabels),
    markerCounts: documents.reduce((aggregate, document) => {
      for (const [marker, count] of Object.entries(document.markerCounts)) {
        aggregate[marker] = (aggregate[marker] || 0) + count;
      }
      return aggregate;
    }, {}),
    danglingNotes: documents
...
function loadCurrentTranslation(filePath, translationId) {
...
function main() {
  ...
  const acquisitionManifestPath = findLatestAcquisitionManifest(
    args.acquisitionManifest,
  );
  const acquisitionManifest = readJson(acquisitionManifestPath);

  if (acquisitionManifest.milestone !== "P05.12A") {
    fail(
      `Expected P05.12A acquisition manifest, found ${acquisitionManifest.milestone}`,
    );
  }

  const sourceById = new Map(
    (acquisitionManifest.downloadedSources || []).map((source) => [
      source.id,
      source,
    ]),
  );
  ...
  const inputManifest = {
    ...
    acquisitionManifest: {
      path: relative(ROOT, acquisitionManifestPath),
      sha256: sha256File(acquisitionManifestPath),
      generatedAtUtc: acquisitionManifest.generatedAtUtc,
    },
    rules: [
      "Read-only census: no generated Scripture data was modified.",
      "No display tokens were rebuilt.",
      "No alignments were modified.",
      "USFM continuation, paragraph, and poetry lines are included in visible verse text.",
      "Headings, footnotes, cross-references, and Strong's metadata are inventoried separately.",
      "Verse bridges and subverses are reported rather than silently rewritten.",
      "KJV conclusions remain provisional pending an independent CrossWire comparison.",
      "Brenton versification conclusions remain provisional pending explicit source-to-reader mapping review.",
    ],
    translations: [],
  };
  ...
  for (const config of configs) {
    console.log(`[P05.12B] Loading current ${config.id.toUpperCase()} reader data...`);
    const sourceConfig = sourceById.get(config.id);
    if (!sourceConfig) {
      fail(`Acquisition manifest has no source for ${config.id}`);
    }

    const sourceDirectory = toAbsoluteRepoPath(sourceConfig.extractedPath);
    if (!fs.existsSync(sourceDirectory)) {
      fail(
        `Extracted source directory not found for ${config.id}: ${sourceDirectory}`,
      );
    }

    const current = loadCurrentTranslation(config.currentPath, config.id);

    console.log(`[P05.12B] Parsing ${config.id.toUpperCase()} USFM...`);
    const source = parseUsfmDirectory(sourceDirectory, config.id);

    console.log(`[P05.12B] Comparing ${config.id.toUpperCase()}...`);
    ...
    inputManifest.translations.push({
      ...
      source: {
        ...
        extractedTreeSha256: sourceConfig.extractedTreeSha256,
        extractedPath: sourceConfig.extractedPath,
        usfmFiles: source.files.length,
      },
    });
  }
  ...
  const overall = {
    milestone: "P05.12B",
    generatedAtUtc: new Date().toISOString(),
    status: "census-complete-no-data-modified",
    translations: Object.fromEntries(
      results.map((result) => [result.summary.translation, result.summary]),
    ),
    totals: {
      currentRecords: results.reduce(
        (sum, result) => sum + result.summary.current.records,
        0,
      ),
      sourceSimpleNumericVerses: results.reduce(
        (sum, result) => sum + result.summary.source.simpleNumericVerses,
        0,
      ),
      substantiveDifferences: results.reduce(
        (sum, result) =>
          sum + result.summary.comparison.substantiveDifferences,
        0,
      ),
      confirmedMissingWordOccurrences: results.reduce(
        (sum, result) =>
          sum + result.summary.comparison.confirmedMissingWordOccurrences,
        0,
      ),
      probableVerseShifts: results.reduce(
        (sum, result) =>
          sum + Number(result.summary.comparison.probableVerseShifts || 0),
        0,
      ),
      headings: results.reduce(
        (sum, result) => sum + result.summary.source.headings,
        0,
      ),
      footnotes: results.reduce(
        (sum, result) => sum + result.summary.source.footnotes,
        0,
      ),
      crossReferences: results.reduce(
        (sum, result) => sum + result.summary.source.crossReferences,
        0,
      ),
      strongTags: results.reduce(
        (sum, result) => sum + result.summary.source.strongTags,
        0,
      ),
    },
    gates: {
      ...
      safeToStartRebuild: false,
      reason:
        "Census findings must be reviewed and source/reader versification maps must be designed before importers rebuild translation data.",
    },
  };
  ...
  const markdown = [
    "# EMETSEES P05.12B External Translation-Integrity Census",
    "",
    ...
    "## Results",
    "",
    "| Translation | Current verses | Source simple verses | Substantive differences | Confirmed missing words | Probable shifts | Source-only books | Current-only books |",
    "|---|---:|---:|---:|---:|---:|---:|---:|",
    ...results.map((result) => {
      const summary = result.summary;
      return `| ${summary.translation.toUpperCase()} | ${summary.current.records} | ${summary.source.simpleNumericVerses} | ${summary.comparison.substantiveDifferences} | ${summary.comparison.confirmedMissingWordOccurrences} | ${summary.comparison.probableVerseShifts} | ${summary.bookInventory.sourceOnlyBooks.length} | ${summary.bookInventory.currentOnlyBooks.length} |`;
    }),
    "",
    "## Interpretation",
    "",
    "- Exact and typography-only matches are separated from substantive differences.",
    "- Confirmed missing-word totals include missing verses and high-confidence shortened text; they do not pretend every changed word is an omission.",
    "- Likely shifts are diagnostic candidates only. No verse has been renumbered.",
    "- Verse bridges and subverses are retained as unresolved source-versification evidence.",
    "- KJV remains provisional until independently compared with CrossWire.",
    "- Brenton requires a dedicated LXX source-to-reader versification map before any rebuild.",
    "",
    "## Build gate",
    "",
    "**FAIL CLOSED:** Do not rebuild translations or resume alignment work from this report alone.",
    "",
  ].join("\n");
  ...
  console.log("");
  console.log("[P05.12B] External translation-integrity census complete.");
  console.log(`OUTPUT_DIR=${args.output}`);
  console.log(
    `[P05.12B] Substantive differences: ${overall.totals.substantiveDifferences}`,
  );
  console.log(
    `[P05.12B] Confirmed missing word occurrences: ${overall.totals.confirmedMissingWordOccurrences}`,
  );
  console.log("[P05.12B] No Bible data or alignments were modified.");
}
