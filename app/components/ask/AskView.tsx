...
import type { AskContextChip } from "./types";

type ScriptureVerse = (typeof sampleVerses)[number];

type SurfaceCount = [string, number];

type HebrewEvidenceOccurrence = {
  reference: string;
  book: string;
  surface: string;
};

type HebrewEvidenceEntry = {
  lemma: string;
  occurrenceCount: number;
  surfaces: SurfaceCount[];
  occurrences: HebrewEvidenceOccurrence[];
};
...
type ConceptEvidence = {
  ...
  hebrewEvidence: Array<HebrewEvidenceEntry | null>;
};

type AskViewProps = {
  initialQuery?: string;
  initialContextChips?: AskContextChip[];
  compact?: boolean;
};
...
function getReferenceHref(verse: ScriptureVerse) {
  return `/read/${encodeURIComponent(verse.book)}/${verse.chapter}?verse=${
    verse.verse
  }`;
}

function savedReaderContextToChip(
  context: BibleIQContext | null
): AskContextChip | null {
  if (!context?.book || !context?.chapter) return null;

  const label = `${context.book} ${context.chapter}${
    context.verse ? `:${context.verse}` : ""
  }`;

  return {
    id: `reader-${context.book}-${context.chapter}-${
...
export default function AskView({
  ...
  const [contextChips, setContextChips] =
    useState<AskContextChip[]>(initialContextChips);
  ...
  useEffect(() => {
    if (initialContextChips.length) return;

    const savedContext = localStorage.getItem("bibleiq-current-context");
    if (!savedContext) return;

    try {
      const chip = savedReaderContextToChip(JSON.parse(savedContext));
      if (chip) setContextChips([chip]);
    } catch {
      setContextChips([]);
    }
  }, [initialContextChips.length]);
  ...
  const conceptEvidence = useMemo<ConceptEvidence | null>(() => {
    if (!hasSearch) return null;
    return getConceptEvidence(submittedSearch) as ConceptEvidence | null;
  }, [hasSearch, submittedSearch]);

  const understood = useMemo(
    () => understandQuestion(submittedSearch),
    [submittedSearch]
  );
  ...
  const fallbackResults = useMemo(() => {
    if (!hasSearch || conceptEvidence) return [];

    const lowerSearch = submittedSearch.toLowerCase();

    return sampleVerses
      .map((verse: ScriptureVerse) => {
        const text = verse.sources
  .map((source: { text: string }) => source.text)
  .join(" ");
      ...
      .slice(0, 25);
  }, [conceptEvidence, hasSearch, submittedSearch, understood.topic]);
      ...
      {hasSearch && !thinking ? (
        <section className="mt-5 rounded-3xl border border-neutral-800 bg-neutral-900/40 p-4">
          ...
          {conceptEvidence ? (
            ...
            <p className="mt-2 text-sm text-amber-300">
              Topic recognized:{" "}
              {understood.topic?.label || understood.topic?.id || "Topic"}
            </p>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <h3 className="mt-1 text-2xl font-bold">{entry.lemma}</h3>
                    </div>
